# Drago
<p align="center">
  <img src="assets/drago mascot (wave).svg" width="70%">
</p>

### AI-Powered Learning Platform for Children with Dyslexia

Drago is an educational platform designed specifically to support **Arabic-speaking children with dyslexia** through personalized learning, artificial intelligence, and interactive educational experiences.

The platform provides a **bilingual interface (Arabic/English)** while delivering educational content tailored to Arabic language learning and dyslexia intervention needs.

Our mission is to make learning more accessible, engaging, and adaptive while addressing reading and phonological challenges through inclusive and research-driven technology.

---

## About Drago

Children with dyslexia in the Arab world still face limited access to specialized educational technology.

Drago was developed as a graduation project to bridge this gap by combining:

* Educational methodologies
* AI-powered assessment
* Accessibility-first design
* Game-based learning
* Personalized learning experiences

---

## Platform Features

### Educational Games

Drago includes four educational games designed to target different dyslexia-related challenges:

* **Volcano Words** — Speech recognition and pronunciation exercises
* **Reading Quest** — Reading comprehension activities
* **Word Hut** — Missing letters and spelling practice
* **Tomb Puzzle** — Sentence and word ordering

### AI & Personalization

* Personalized learning recommendations
* Adaptive learning paths
* Speech-based educational assessment
* Reading and pronunciation error analysis
* NLP-supported educational processing

### Doctor Dashboard

* Student progress tracking
* Session and assessment management
* Real-time monitoring and analytics

---

## Accessibility & Learning Experience

Drago was designed with accessibility at its core:

* Full Arabic RTL support
* OpenDyslexic readable font
* Child-centered UI experience
* Custom Drago mascot and visual identity
* Progressive difficulty levels
* Offline support through PWA

---

## Behind The Project

Throughout the journey, we:

* Studied dyslexia intervention approaches
* Designed a structured learning guide
* Built tailored datasets for educational activities
* Adapted Whisper-based speech recognition for reading exercises
* Applied NLP techniques for intelligent educational feedback
* Discussed requirements and validated ideas with specialists
* Prepared a research paper based on the project

---

## Tech Stack

### Frontend & User Experience

* React 19
* Vite
* Progressive Web App (PWA)
* Offline Support
* RTL Arabic Support
* Custom Drago Mascot & Accessible UI
* OpenDyslexic/ Maqroo Font
* Vercel Deployment

---

### Backend & Infrastructure

* ASP.NET
* SignalR (Real-time Communication)
* REST APIs
* Monster API (AI Model Hosting & Inference)

---

### AI & Machine Learning

* Customized Whisper-based Speech Recognition
* Hugging Face Models
* Natural Language Processing (NLP)
* Educational Error Detection
* AI Recommendation Engine

---

### Data & Learning Engineering

* Tailored Educational Datasets
* Dataset Generation & Validation
* Progressive Difficulty Design
* Learning Analytics & Progress Tracking

---

## Explore Drago

🌐 Live Demo → https://drago-frontend.vercel.app/
---
🎥 Demo Video → https://drive.google.com/file/d/1koRrGcUncSmSUndIKFz_VvVOUcY03Cq4/view?usp=sharing
---
Presentation link → https://canva.link/g7bnbcn63umh5bl
---

## Team

Graduation Project Team

### Supervisors

* Dr. Marwa Samara
* Dr. Asmaa Ali
* Dr. Nihal Mabrok

---

### Team Leader

* Habiba Mohamed

---

### Frontend Team

* Habiba Mohamed
* Mazen Mohamed
* Sara Medhat
* Yousef Hussein

---

### Backend Team

* Alaa Moustafa
* Emy Ayman
* Steven Adly

---

### AI Team

* Mohamed Ahmed
* Maria Emad
* Samy Georges
* Toqa Yasser

---

We built Drago through collaborative work across educational research, AI development, accessibility design, and full-stack engineering.


---

Technology becomes more meaningful when it creates opportunities for everyone.
